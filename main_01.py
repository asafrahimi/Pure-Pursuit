...
def vehicle_location(): # 1 Determine the current position of the vehicle
    return        
...
def set_steering(states): #4
    ...
    delta = math.atan(2*L*math.sin(states.alpha) / Ld)    
    return delta, ind 
...
def main():
    ...
    while  (time < 30) and (ind < len(cx)): 
        ...
        states = update_vehicle_position(states, calc_delta[0])
        time = time + dt
